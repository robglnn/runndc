...
- Recent enhancements: local openFDA dataset (135k NDCs) + AI fallback to suggest NDCs when live FDA queries fail (now works even without OpenAI by using deterministic scoring), 4-4 product fallback, package list display, drug name surfacing, warnings for non-standard units/inactive packages, variant RxNorm lookups (e.g., inhalers), unparsed package surfaced for manual review, guardrails around FDA/RxNorm/AI parsing so production never 500s, explicit inactive NDC warnings by cross-referencing the local FDA snapshot when live lookups 404, a high-priority inactive banner in the UI so pharmacists immediately see expired codes, embedding the FDA snapshot as a base64 asset so serverless builds always have access, tightened AI/local-index ranking so text-only requests honor dosage form/route/ingredient constraints (Metformin tablets no longer map to IV bags) while API responses now prefer RxNorm/generic product labels over raw ingredient strings, inhaler-specific parsing so aerosol/metered packages convert to usable quantities (plus UI removal of the AI confidence badge per user request), and a follow-up local-index cross-check that restores the inactive banner even when FDA parsing succeeds but only expired packages remain.
